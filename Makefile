...
ifeq ($(ROOTSIZE),0)
	...
	virt-copy-in -a $(IMGDIR)/$(SNAME)/rootfs.qcow2 puppet-enterprise-2019.2.1-ubuntu-18.04-amd64.tar.gz /opt/
	virt-copy-in -a $(IMGDIR)/$(SNAME)/rootfs.qcow2 pe.conf /opt/
	virt-copy-in -a $(IMGDIR)/$(SNAME)/rootfs.qcow2 user_data.conf /opt/
